...
export function getPrefectureFromCoords(lat: number, lng: number): PrefectureName {
  let bestName: PrefectureName = PREFECTURES[0].name
  let bestDist = Infinity
  for (const pref of PREFECTURES) {
    const dLat = lat - pref.lat
    const dLng = lng - pref.lng
